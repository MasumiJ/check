...
import market.dataanalysistool.jpa.MarketIndicators;
...
public class StockStdAndPoorSessionBean implements StockStdAndPoorSessionBeanRemote, StockStdAndPoorSessionBeanLocal {
	...
	public StockInfo getStockInfo(String ticker) {
		...
		// .createQuery("SELECT m from Market as m where m.id.ticker_ = '" +
		// ticker + "'");

		@SuppressWarnings("unchecked")
		TypedQuery<Object[]> query = (TypedQuery<Object[]>) em
				.createQuery("SELECT m.open_,m.high_,m.low_,m.close_,m.volume FROM Market m  WHERE m.id.ticker_ = '"
						+ ticker + "' ORDER BY m.id.x_Date DESC");
		List<Object[]> obj = (List<Object[]>) query.getResultList();

		List<MarketIndicators> marketIndicatorList = new ArrayList<MarketIndicators>();

		MarketIndicators marketIndicators = new MarketIndicators();
		marketIndicators.setOpen_((Double) obj.get(0)[0]);
		marketIndicators.setHigh_((Double) obj.get(0)[1]);
		marketIndicators.setLow_((Double) obj.get(0)[2]);
		marketIndicators.setClose_((Double) obj.get(0)[3]);
		marketIndicators.setVolume((int) obj.get(0)[4]);

		stockInfo.setMarketIndicators(marketIndicators);
		
		Double bd = get52WeekHigh(ticker);

		return stockInfo;
	}

	public Double get52WeekHigh(String ticker) {

		@SuppressWarnings("unchecked")
		TypedQuery<Double[]> query = (TypedQuery<Double[]>) em
				.createQuery("SELECT MAX(m.high_) from Market m WHERE m.id.ticker_ =:ticker");
		query.setParameter("ticker", ticker);

		List<Double[]> max52WeekList = (List<Double[]>) query.getResultList();
		
		System.out.println((Double)max52WeekList.get(0)[0]);
		return (Double)max52WeekList.get(0)[0];
	}
